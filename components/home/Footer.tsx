...
// --- FIXED CLOCK: Green Radar Sweep + White Hands + 60 Ticks ---
const ModernClock = () => {
  ...
  return (
    <div className="relative group scale-75 md:scale-100 origin-center md:origin-right">
...
const AutoFlickerLetter = ({ char }: { char: string }) => {
    ...
    useEffect(() => {
        ...
        const loop = () => {
            if (Math.random() > 0.7) { setIsHollow(true); setTimeout(() => setIsHollow(false), 50 + Math.random() * 150); }
    ...
    return (
        <span className="inline-block transition-none cursor-default select-none" style={{ color: isHollow ? 'transparent' : 'white', WebkitTextStroke: isHollow ? '1px #CCFF00' : '0px transparent', opacity: isHollow ? 0.8 : 1 } as React.CSSProperties}>
            {char}
        </span>
    )
}
  ...
  return (
    <footer className="bg-black border-t border-white/10 relative overflow-hidden">
        ...
        <div className="max-w-[1400px] mx-auto px-8 py-20 md:py-32 grid grid-cols-1 md:grid-cols-4 gap-12 md:gap-16 border-t border-white/10">
           ...
           <div className="md:col-span-1 flex flex-col justify-between h-full gap-8">
              <div>
                 ...
                 <div className="font-mono text-xs text-white/50 leading-relaxed space-y-4">
                    {/* Fixed HTML nesting error: div instead of span */}
           ...
           {/* COL 4: CLOCK */}
           <div className="md:col-span-1 flex flex-col items-center md:items-end justify-center py-8 md:py-0"><ModernClock /></div>
        </div>
      </footer>
  );
}