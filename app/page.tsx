...
// --- FIX 1: Internal Wrap Function (No external dependency) ---
...
const BrutalLabel = ({ children }: { children: React.ReactNode }) => (
  <span className="font-mono text-[10px] md:text-xs font-black tracking-[0.2em] md:tracking-[0.3em] uppercase text-white/70 text-shadow-[0_1px_2px_black] flex items-center gap-2">
    {children}
  </span>
);
...
export default function Home() {
  ...
  useGSAP(() => {
    ...
    mm.add("(max-width: 767px)", () => {
      if (slidesRef.current) {
        gsap.set(slidesRef.current, { xPercent: 0 });
      }
    });
  }, { scope: container });
  ...
  return (
    <main ref={container} className="relative bg-black selection:bg-accent selection:text-black w-full overflow-x-hidden" onMouseMove={handleMouseMove}>
      ...
      {/* TAPE 1 */}
      <div className="border-y border-white/10 bg-[#050505] py-6 md:py-8 overflow-hidden relative z-20">
        <ParallaxText baseVelocity={-2}>
           <div className="flex items-center gap-12 md:gap-24 opacity-50 text-white">
      ...
      <section id="method" className="py-20 md:py-40 px-4 md:px-20 bg-[#080808] relative z-20 overflow-hidden">
        ...
        <motion.div className="absolute w-[300px] h-[300px] md:w-[600px] md:h-[600px] bg-accent/5 rounded-full blur-[100px] pointer-events-none -translate-x-1/2 -translate-y-1/2 mix-blend-screen" style={{ left: mouseX, top: mouseY }} />
        <div className="max-w-7xl mx-auto relative z-10">
          <div className="mb-20 md:mb-32 flex flex-col md:flex-row md:items-end justify-between gap-6 border-b border-white/10 pb-12 relative group">
             ...
             <div className="relative">
                ...
                <div className="relative overflow-hidden py-2">
                   <h2 className="text-6xl md:text-[10rem] font-black tracking-tighter font-syncopate relative z-20 leading-[0.8] select-none text-white">
          ...
          <div className="grid grid-cols-1 lg:grid-cols-12 gap-8 lg:gap-16">
            ...
            <motion.div className="lg:col-span-8 relative h-[400px] md:h-[600px] z-20">
               <div className="w-full h-full bg-black border border-white/10 shadow-2xl overflow-hidden relative">
                  ...
                  <div className="absolute inset-0 z-10 p-8 md:p-12 flex flex-col justify-end">
                      ...
                      <div className="relative mb-12">
                        ...
                        <h3 className="font-syncopate text-4xl md:text-7xl font-black text-white leading-[0.9] mb-8 italic mix-blend-screen min-h-[3.6rem]">{protocols[activeMethod].fullTitle}</h3>
                        <div className="font-mono text-xs md:text-base text-white/80 leading-relaxed max-w-lg border-l-4 pl-6 py-2 bg-black/40 backdrop-blur-sm min-h-[5rem]" style={{ borderColor: protocols[activeMethod].color }}>{protocols[activeMethod].content}</div>
                      </div>
      ...
      {/* SPACE */}
      <section ref={spaceSectionRef} id="space" className="min-h-screen h-auto md:h-screen bg-[#050505] relative z-20 flex flex-col md:flex-row overflow-hidden border-t border-white/5">
        ...
        <div className="w-full md:w-1/2 h-auto md:h-full flex flex-col justify-center relative z-20 bg-[#050505] border-r border-white/10 py-20 md:py-0">
           <div className="absolute top-12 right-12 text-right font-mono text-[10px] text-white/50 tracking-widest z-30 hidden md:block">COORDS: 34.05.{activeSlide}1 // 118.24.{9 - activeSlide}5<br/>SECTOR: 0{activeSlide}</div>
           ...
           <div className="absolute bottom-12 left-12 flex items-end gap-6 hidden md:flex">
        ...
        <div className="w-full md:w-1/2 h-auto md:h-full overflow-hidden relative">
           <div ref={slidesRef} className="flex flex-col md:flex-row h-auto md:h-full w-full md:w-[400%] will-change-transform">
              {slides.map((slide, i) => (
                 <div key={i} className="w-full md:w-1/4 h-[50vh] md:h-full flex flex-col justify-center px-8 md:px-20 border-b md:border-b-0 md:border-r border-white/5 relative bg-[#050505]/50 backdrop-blur-sm">
                    ...
                    <div className="relative z-10">
                       ...
                       <h3 className="font-syncopate text-3xl md:text-5xl font-black text-white uppercase mb-4">{slide.title}</h3>
          ...
          <div className="flex flex-col md:flex-row justify-between items-end mb-16 md:mb-24">
            <div>
              ...
              <div className="relative mt-4">
                <h2 className="text-6xl md:text-9xl font-black tracking-tighter font-syncopate relative z-10 leading-[0.85] select-none text-white">
            ...
            <div className="flex items-center gap-2 mt-8 md:mt-0 p-1 border border-white/20 rounded-lg bg-white/5 backdrop-blur-sm relative self-start md:self-end">
                <motion.div 
                  ...
                  style={{ left: billingCycle === 'monthly' ? '4px' : '50%', right: billingCycle === 'monthly' ? '50%' : '4px', width: 'calc(50% - 6px)' }}
                />
                <button onClick={() => setBillingCycle('monthly')} className={`relative z-10 px-6 md:px-8 py-2 md:py-3 font-mono text-xs font-bold tracking-widest transition-colors duration-300 ${billingCycle === 'monthly' ? 'text-black' : 'text-white/50'}`}>MONTHLY</button>
                <button onClick={() => setBillingCycle('annual')} className={`relative z-10 px-6 md:px-8 py-2 md:py-3 font-mono text-xs font-bold tracking-widest transition-colors duration-300 ${billingCycle === 'annual' ? 'text-black' : 'text-white/50'}`}>ANNUAL</button>
            </div>
          </div>
                ...
                <SpotlightCard key={i} color={tier.color} className={isDimmed ? 'opacity-40 blur-sm scale-95' : ''}>
                  <div onMouseEnter={() => setHoveredCard(i)} className={`p-8 md:p-10 h-full flex flex-col relative transition-all duration-500`}>
                    ...
                    <div className="flex items-baseline gap-1 mb-8">
                      <AnimatePresence mode='wait'><motion.span key={billingCycle} initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -10 }} className="text-6xl font-black text-white tracking-tighter">${billingCycle === 'monthly' ? tier.price.m : tier.price.a}</motion.span></AnimatePresence>
                      <span className="text-sm font-mono text-white/50">/MO</span>
                    </div>
                    <ul className="space-y-4 mb-12 relative z-10 flex-grow">{tier.features.map((feat, idx) => (<li key={idx} className="flex items-start gap-3 text-xs font-mono text-white/70"><Check className="w-3 h-3 text-accent mt-0.5 shrink-0" /> {feat}</li>))}</ul>
                    <button className={`w-full py-4 font-mono text-xs font-bold tracking-widest uppercase transition-all duration-300 relative z-10 ${tier.popular ? 'bg-accent text-black hover:bg-white' : 'border border-white/20 text-white hover:bg-white hover:text-black'}`}>SELECT PLAN</button>
                  </div>
                </SpotlightCard>
              );
            })}
      ...
      <footer className="bg-black border-t border-white/10 relative overflow-hidden">
        <MarqueeStrip direction="left" items={["HYPERTROPHY PROTOCOL", "MAXIMAL EFFORT", "ATP REGENERATION", "MITOCHONDRIAL DENSITY", "CNS POTENTIATION", "VELOCITY BASED TRAINING"]} className="border-none bg-transparent" />
        
        <div className="max-w-[1400px] mx-auto px-8 py-20 md:py-32 grid grid-cols-1 md:grid-cols-12 gap-12 md:gap-16 border-t border-white/10">
           ...
           <div className="md:col-span-4 flex flex-col justify-between h-full gap-8">
              <div>
                 <div className="mb-8 cursor-default group select-none">
                    <span className="font-syncopate font-black text-4xl md:text-5xl text-white flex flex-wrap">
                 ...
                 <div className="font-mono text-xs text-white/50 leading-relaxed space-y-4">
                    <div className="flex items-center gap-3"><div className="w-8 h-8 rounded bg-white/5 flex items-center justify-center border border-white/10"><MapPin className="w-4 h-4 text-accent"/></div><div>SECTOR 07<br/>INDUSTRIAL DISTRICT</div></div>
                    <div className="flex items-center gap-3"><div className="w-8 h-8 rounded bg-white/5 flex items-center justify-center border border-white/10"><Mail className="w-4 h-4 text-accent"/></div><div>SYSTEM@KINETIC.GYM<br/>ENCRYPTED_CHANNEL</div></div>
                 </div>
              </div>
           ...
           <div className="md:col-span-2">
              ...
              <ul className="space-y-4 font-mono text-sm text-white/70">
                 {['MANIFESTO', 'LOCATIONS', 'SHOP_GEAR', 'CAREERS', 'FRANCHISE'].map((item) => <li key={item} className="hover:text-white cursor-pointer transition-colors flex items-center gap-2 group"><span className="w-1 h-1 bg-white/20 group-hover:bg-accent transition-colors"/> {item}</li>)}
              </ul>
           </div>
           ...
           {/* CLOCK */}
           <div className="md:col-span-3 flex flex-col items-center md:items-end justify-center py-8 md:py-0">
              <ModernClock />
           </div>
        </div>
      </footer>
