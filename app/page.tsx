...
// --- 1. QUANTUM ANALOG CLOCK (BIGGER & BETTER) ---
const QuantumClock = () => {
  ...
  return (
    <div className="relative group">
        ...
        <div className="w-64 h-64 relative flex items-center justify-center rounded-full bg-[#080808] border border-white/10 shadow-2xl">
           ...
           {[...Array(60)].map((_, i) => {
              ...
              return (
                <div 
                    key={i} 
                    className={`absolute origin-bottom bottom-1/2 left-1/2 -translate-x-1/2 ${isHour ? 'w-0.5 h-4 bg-white' : 'w-[1px] h-2 bg-white/20'}`} 
           ...
           {/* Hour Hand */}
           <div className="absolute w-1.5 h-16 bg-white rounded-full origin-bottom bottom-1/2 left-1/2 -translate-x-1/2 z-10 shadow-lg" 
                style={{ transform: `translateX(-50%) rotate(${hours * 30 + minutes * 0.5}deg)` }} />
           
           {/* Minute Hand */}
           <div className="absolute w-1 h-24 bg-white/70 rounded-full origin-bottom bottom-1/2 left-1/2 -translate-x-1/2 z-20" 
                style={{ transform: `translateX(-50%) rotate(${minutes * 6}deg)` }} />
           
           {/* Second Hand (Lime Accent) */}
           <div className="absolute w-0.5 h-28 bg-[#CCFF00] origin-bottom bottom-1/2 left-1/2 -translate-x-1/2 z-30 shadow-[0_0_15px_#CCFF00]" 
                style={{ transform: `translateX(-50%) rotate(${seconds * 6}deg)` }} />
           
           {/* Center Cap */}
           <div className="absolute w-3 h-3 bg-[#CCFF00] rounded-full z-40 border-2 border-black" />
           <div className="absolute w-1 h-1 bg-black rounded-full z-50" />
        </div>
    </div>
  );
};

// --- 2. AUTOMATIC NEURAL FLICKER (FOOTER LOGO) ---
// Flickers randomly without hover interaction
const AutoFlickerLetter = ({ char }: { char: string }) => {
    ...
    useEffect(() => {
        ...
        const loop = () => {
            // 20% chance to flicker to hollow state
            const shouldFlicker = Math.random() > 0.8; 
            
            if (shouldFlicker) {
                setIsHollow(true);
                // Stay hollow for 100-300ms
                setTimeout(() => setIsHollow(false), 100 + Math.random() * 200); 
            }
            
            // Try again in 200-1500ms
            timeout = setTimeout(loop, 200 + Math.random() * 1300); 
        };
        loop();
    ...
    return (
        <span 
            ...
            style={isHollow ? { color: 'transparent', WebkitTextStroke: '2px #CCFF00' } : { color: 'white', WebkitTextStroke: '0px transparent' }}
        >
            {char}
        </span>
    )
...
function ParallaxText({ children, baseVelocity = 100 }: ParallaxProps) {
  ...
  return (
    <div className="overflow-hidden m-0 flex flex-nowrap whitespace-nowrap">
      <motion.div className="flex flex-nowrap gap-24 items-center" style={{ x }}>
...
const MarqueeStrip = ({ items, direction = "left", speed = 20, className = "" }: { items: React.ReactNode[], direction?: "left" | "right", speed?: number, className?: string }) => {
  return (
    <div className={`w-full overflow-hidden border-y border-white/10 bg-[#050505] py-4 ${className}`}>
        ...
        >
          {[...Array(4)].map((_, i) => (
            <React.Fragment key={i}>
              {items.map((item, idx) => (
                <div key={`${i}-${idx}`} className="flex items-center gap-4 text-white/40 font-mono text-xs font-bold tracking-[0.2em]">
                  {item}
                </div>
              ))}
            </React.Fragment>
...
const DecryptedText = ({ text, className = "", speed = 30, reveal = true, hollow = false }: { text: string, className?: string, speed?: number, reveal?: boolean, hollow?: boolean }) => {
  ...
  return <span className={className} style={hollow ? { WebkitTextStroke: '1px #fff', color: 'transparent' } : {}}>{display}</span>;
};
...
function SpotlightCard({ children, className = "", color = "rgba(204, 255, 0, 0.2)" }: { children: React.ReactNode; className?: string, color?: string }) {
  ...
  return (
    <div
      className={`group relative border border-white/10 bg-[#0a0a0a] overflow-hidden transition-all duration-300 hover:scale-105 hover:z-10 hover:shadow-2xl hover:shadow-black/50 ${className}`}
...
const Magnetic = ({ children }: { children: React.ReactNode }) => {
    ...
    return <motion.div onMouseMove={handleMouseMove} onMouseLeave={handleMouseLeave} style={{ x: springX, y: springY }}>{children}</motion.div>;
};
...
export default function Home() {
  ...
  return (
    <main ref={container} className="relative bg-black selection:bg-accent selection:text-black" onMouseMove={handleMouseMove}>
      ...
      <footer className="bg-black border-t border-white/10 relative overflow-hidden">
        ...
        <div className="max-w-[1400px] mx-auto px-8 py-32 grid grid-cols-1 md:grid-cols-4 gap-16 border-t border-white/10">
           ...
           {/* COL 4: QUANTUM CLOCK (Dedicated Column) */}
