...
function SpotlightCard({ children, className = "", color = "rgba(204, 255, 0, 0.2)" }: { children: React.ReactNode; className?: string, color?: string }) {
  ...
  // OPTIMIZATION: Store rect in ref to avoid recalculating on every pixel move
  ...
  return (
    <div
      className={`group relative border border-white/10 bg-[#0a0a0a] overflow-hidden transition-all duration-300 active:scale-95 md:hover:scale-105 md:hover:z-10 md:hover:shadow-2xl md:hover:shadow-black/50 ${className}`}
      onMouseEnter={handleMouseEnter} // Calc bounds once on enter
      onMouseMove={handleMouseMove}   // Use cached bounds
    >
...
export default function Home() {
  ...
  return (
    <main ref={container} className="relative bg-black selection:bg-accent selection:text-black w-full overflow-x-hidden" onMouseMove={handleMouseMove}>
      ...
      {/* REPLACED SVG WITH CSS CLASS */}
      <div className="fixed inset-0 z-[100] bg-noise" />
      ...
      <section id="hero" className="relative w-full bg-black overflow-hidden md:h-screen">
        ...
        <div className="hidden md:flex absolute inset-0 flex-col items-center justify-center overflow-hidden">
          ...
          {/* OPTIMIZED: Only render 3D on desktop */}
          <div className="hidden md:block absolute inset-0 z-10 opacity-80 pointer-events-none scale-100 origin-center">
            <FuturisticShape />
          </div>
      ...
      <section ref={spaceSectionRef} id="space" className="min-h-screen bg-[#050505] relative z-20 flex flex-col md:flex-row overflow-hidden border-t border-white/5">
        ...
        {/* OPTIMIZED: Use local noise class instead of external SVG */}
        <div className="absolute inset-0 z-0 pointer-events-none opacity-30 bg-noise mix-blend-overlay" />
